...
export function useListHookPoints(
  options?: UseQueryOptions<scriptservicev1_ListHookPointsResponse, Error>
) {
  return useQuery({
    queryKey: ["listHookPoints"],
    queryFn: () => apiClient.scriptService.ListHookPoints({}),
...
export const scriptLanguageList = computed(() => [
  { value: "LUA", label: t("pages.script.languageLua") },
  { value: "JAVASCRIPT", label: t("pages.script.languageJavascript") },
]);
...
const SCRIPT_LANGUAGE_TAG_MAP: Record<string, string> = {
  LUA: "primary",
  JAVASCRIPT: "warning",
  DEFAULT: "info",
};

export function scriptLanguageToTagType(language: any): string {
  return SCRIPT_LANGUAGE_TAG_MAP[language as string] || SCRIPT_LANGUAGE_TAG_MAP.DEFAULT;
}
