...
| L2 输入 | `bgInset` | `#161F33` | 输入框/下拉框（比表面微亮，"浮起"式） |
| 浮层 | `bgElevated` | `#1C2128` | 下拉、弹窗、Popover |
| 表头 | — | `#1F2937` | 表格表头独立次级色 |

配套（暗色必读，均为定稿结论，勿改）：

- **边框要"拒绝隐形"**：输入类 `rgba(148,163,184,.22)`（hover `.4`，focus 主色）、通用 `rgba(148,163,184,.28)`、分隔线 `rgba(255,255,255,.08)`（5% 时卡片与大底糊成一片）。输入类边框必须带蓝相（slate），纯白 α 边框在蓝黑底上显脏灰。
- **文字**：主 `#F8FAFC`、次 `#8B949E`、三级 `#6E7681`、placeholder `#64748B`；表单标签降一档用 `#DCE3ED`（值是主角）。
...
- **表单/抽屉**：输入控件底 L2（暗）/白（浅）；focus 主色边框 + 3px 12% 柔光；抽屉遮罩 `rgba(0,0,0,.6)`，宽度基准 480。
