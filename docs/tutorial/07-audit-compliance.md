...
不需要改代码或环境变量（环境变量配置方式已废弃）。

## 4. 管理页在哪

六类审计日志各有独立查询页（三端齐备，当前形态为列表 + 筛选 + 导出）。记录携带
trace_id，可与链路追踪（Jaeger）对账。参数管理 / 登录策略 / 在线用户（强制下线）在
系统管理对应的管理页。
...
- `before_data`/`after_data`（操作审计的改前改后快照）与 `table_name`/`db_user`（数据访问审计）
  留空待增强——当前版本没有这两块，查不到不是配置问题；
